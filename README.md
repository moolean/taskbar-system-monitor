...
- 同时限制传统任务列表并裁剪 Windows 11 XAML 合成层，应用按钮变多时也不会与监控文字重叠
- 默认实时采样任务栏合成色，形成无边框的“无缝任务栏背景”，不会露出下层应用按钮
- 无缝模式使用逐像素 Alpha 图层，平滑字体边缘不会产生色键重影
- 右键可切换“无缝任务栏背景”或“固定系统深浅色”，选择会自动保存
...
- 任务栏数值条默认无底色；右键打开“任务栏背景”可切换透明或系统主题背景。
- 左键单击任务栏数值条：打开详细监控面板。
- 右键单击任务栏数值条：打开设置菜单。
